...
# This script normalizes the GlobalPhone Mandarin dictionary. 
# It (optionally) 
# tags the phones with language ('CH') marker. 
# It also converts the words to  UTF8 and lowercases everything, either of which can be diabled with command 
# line switches.
# No special treatment for acronyms since there is no easy way of detecting 
# acronyms in the dictionary.

my $usage = "Usage: gp_norm_dict_CH.pl [-l|-m map|-u] -i dictionary > formatted\
Normalizes pronunciation dictionary for GlobalPhone Hausa.\
...
while (<L>) {
  ...
  $pron =~ s/H_//g;    # Get rid of the M_ marker before the phones
